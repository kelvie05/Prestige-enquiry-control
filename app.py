...
SEED_USERS = [
    ...
    ("u4", "Dave Holt",   "Sales/Admin", "#F16522", "dave",  "dave"),
...
# Roles an Admin can assign in the Users screen. Everyone can see and progress
# every enquiry; the role only adds powers (Admin manages users + can override a
# duplicate SigmaMRP number; Management approves high-value quotes).
ALLOWED_ROLES = ["Admin", "Estimator", "Sales/Admin", "Management", "Production Manager"]
AVATAR_COLOURS = ["#3B72C9", "#2E8F66", "#F16522", "#7A5BD0", "#CF4B45", "#D69A2D", "#0C8C9E"]
...
def init_db():
    ...
    with db() as conn:
        ...
        # migration: ensure 'active' column exists on databases created earlier
        ...
        for uid, name, role, colour, username, default_pw in SEED_USERS:
            ...
            if existing is None:
                # if the preferred id is already taken (e.g. an older account
                # reused the same id), allocate a fresh unique id instead of crashing
                new_id = uid
                if conn.execute("SELECT 1 FROM users WHERE id=?", (new_id,)).fetchone():
                    n = 1
                    while conn.execute("SELECT 1 FROM users WHERE id=?", ("u" + str(n),)).fetchone():
                        n += 1
                    new_id = "u" + str(n)
                ...
                conn.execute(
                    ...
                    " VALUES(?,?,?,?,?,?,?)",
                    (new_id, name, role, colour, username, ph, salt),
                )
            else:
                conn.execute(
...
def user_admin(row):
    """Fuller view for the admin Users screen."""
    return {"id": row["id"], "name": row["name"], "role": row["role"], "c": row["colour"],
...
class Handler(BaseHTTPRequestHandler):
        ...
        with _db_lock, db() as conn:
            ...
            if path == "/api/users":
                ...
                if not u:
                    return self._err(401, "not logged in")
                if u["role"] != "Admin":
                    return self._err(403, "admins only")
                rows = conn.execute("SELECT * FROM users ORDER BY active DESC, name").fetchall()
                return self._json({"users": [user_admin(r) for r in rows],
                                   "roles": ALLOWED_ROLES})
    ...
    def api_post(self, path):
        ...
        if path == "/api/login":
            with _db_lock, db() as conn:
                ...
                if not row["active"]:
                    return self._err(403, "This account has been deactivated")
        ...
        with _db_lock, db() as conn:
            ...
            if path == "/api/users":
                ...
                if not (name and username and password):
                    return self._err(400, "name, username and password are required")
                if role not in ALLOWED_ROLES:
                    return self._err(400, "invalid role")
                if not re.match(r"^[a-z0-9._-]+$", username):
                    return self._err(400, "username may use letters, numbers, dot, dash, underscore")
                if conn.execute("SELECT 1 FROM users WHERE username=?", (username,)).fetchone():
                    return self._err(409, "that username is already taken")
                uid = next_user_id(conn)
                colour = AVATAR_COLOURS[sum(ord(c) for c in username) % len(AVATAR_COLOURS)]
                ph, salt = hash_pw(password)
                conn.execute(
                    "INSERT INTO users(id,name,role,colour,username,pw_hash,pw_salt,active)"
                    " VALUES(?,?,?,?,?,?,?,1)",
                    (uid, name, role, colour, username, ph, salt),
                )
                conn.commit()
                return self._json({"user": {"id": uid, "name": name, "role": role,
                                            "c": colour, "username": username, "active": True}})
    ...
    def api_put(self, path):
        ...
        # ---- admin: edit a user account ----
        mu = re.match(r"^/api/users/(u\w+)$", path)
        if mu:
            with _db_lock, db() as conn:
                ...
                if me["role"] != "Admin":
                    return self._err(403, "admins only")
                target_id = mu.group(1)
                target = conn.execute("SELECT * FROM users WHERE id=?", (target_id,)).fetchone()
                if not target:
                    return self._err(404, "no such user")

                # work out the resulting role / active so we can guard against lock-out
                new_role = (body.get("role") or target["role"]).strip()
                if new_role not in ALLOWED_ROLES:
                    return self._err(400, "invalid role")
                new_active = body.get("active", bool(target["active"]))

                # never let the last active admin be demoted or deactivated, and
                # never let an admin lock themselves out
                was_admin = target["role"] == "Admin" and target["active"]
                still_admin = (new_role == "Admin") and new_active
                if was_admin and not still_admin:
                    others = conn.execute(
                        "SELECT COUNT(*) c FROM users WHERE role='Admin' AND active=1 AND id!=?",
                        (target_id,)).fetchone()["c"]
                    if others == 0:
                        return self._err(400, "You can't remove the last active admin.")
                if target_id == me["id"] and not new_active:
                    return self._err(400, "You can't deactivate your own account.")

                # username change (optional)
                new_username = body.get("username")
                if new_username is not None:
                    new_username = new_username.strip().lower()
                    if not re.match(r"^[a-z0-9._-]+$", new_username or ""):
                        return self._err(400, "invalid username")
                    clash = conn.execute("SELECT 1 FROM users WHERE username=? AND id!=?",
                                         (new_username, target_id)).fetchone()
                    if clash:
                        return self._err(409, "that username is already taken")
                else:
                    new_username = target["username"]

                new_name = (body.get("name") or target["name"]).strip()
                conn.execute(
                    "UPDATE users SET name=?, role=?, username=?, active=? WHERE id=?",
                    (new_name, new_role, new_username, 1 if new_active else 0, target_id))
                ...
                return self._json({"user": user_admin(row)})
