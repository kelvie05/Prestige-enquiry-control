...
def init_db():
    ...
    with db() as conn:
        ...
        # seed users (password can be overridden per-user via env, see env_pw)
        if not conn.execute("SELECT 1 FROM users LIMIT 1").fetchone():
            for uid, name, role, colour, username, pw in SEED_USERS:
                ph, salt = hash_pw(env_pw(username, pw))
                conn.execute(
                    "INSERT INTO users(id,name,role,colour,username,pw_hash,pw_salt)"
                    " VALUES(?,?,?,?,?,?,?)",
                    (uid, name, role, colour, username, ph, salt),
                )
        # rotate any password supplied via env on every boot (so you can change a
        # password by updating the variable in Railway and re-deploying)
        for uid, name, role, colour, username, pw in SEED_USERS:
...
def env_pw(username, default):
    """Resolve a seed user's initial password, allowing an env override
    (PF_PW_MARK, PF_PW_SARAH, ...). Lets you avoid the well-known demo
    passwords on a public deployment."""
    return os.environ.get("PF_PW_" + username.upper(), default)
        ...
        with _db_lock, db() as conn:
            ...
            if path == "/api/state":
                ...
                return self._json({
                    ...
                    "suppliers": suppliers,
                    "needsSeed": (not seeded) and len(enquiries) == 0,
                })
...
def main():
    ...
    print("=" * 62)
    print("  Prestige Fabrications - Enquiry Control  (Phase 2)")
    print("=" * 62)
    print(f"  Running at:   http://localhost:{PORT}")
    print(f"  Data dir:     {DATA_DIR}")
    print(f"  Database:     {DB_PATH}")
    print(f"  File storage: {UPLOAD_DIR}")
    print("  Logins:       sarah / mark / priya / dave / janet")
    print("                (password = username for this prototype)")
