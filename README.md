...
### Adding or changing accounts (the easy way)

Sign in as an **Admin** and use the **Users** screen in the left sidebar (only Admins see it).
From there you can, with no file editing or redeploys:

- **Add a person** — name, username, role, and a starting password. They can sign in straight away.
- **Edit** anyone — change their name, role, or username, or **reset their password**.
- **Deactivate / reactivate** an account — a deactivated person can't sign in, but their name and
  history stay intact on past enquiries. (You can't deactivate yourself or the last Admin.)

Roles are `Admin`, `Estimator`, `Sales/Admin`, `Management`, `Production Manager`. Everyone can see
and progress every enquiry; the role only adds powers — Admin manages users and can override a
duplicate SigmaMRP number, and Management approves high-value quotes. Deactivated people are hidden
from the "assign to" menus automatically.

### The starter Admin account (in the file)

You still need one account to log in with the very first time. That's seeded from the `SEED_USERS`
list near the top of `app.py`. The simplest setup is to leave one Admin there, set its password with
a `PF_PW_<USERNAME>` variable (below), and add everyone else from the Users screen. The placeholder
accounts that ship with it can be edited or deactivated once you're in.

### Setting passwords from the file

For the seeded account(s), set the password as a Railway **Variable** named `PF_PW_` + the username
in capitals (e.g. `PF_PW_SARAH`). This keeps it out of your code. Passwords for people you add via
the Users screen are set right there in the app — no variable needed.
