...
Each account is one line in the `SEED_USERS` list near the top of `app.py`:

```python
#  id     name            role            colour      username  password
("u6", "Tom Webb",   "Estimator",   "#0C8C9E", "tom",  "tom"),
```

- **username** is what they type to log in (lowercase, no spaces).
- **role** is one of `Admin`, `Estimator`, `Sales/Admin`, `Management`. Everyone can see and
  progress every enquiry; the role only adds powers — Admin can override a duplicate SigmaMRP
  number, Management can approve high-value quotes.
- **id** must be unique (`u6`, `u7`, …). The **colour** is any hex value for their avatar.
- The **password** in the file is only a fallback — on a live site set `PF_PW_<USERNAME>`
  instead (see below) so passwords aren't stored in your code.

Add or edit lines, push to GitHub, and Railway redeploys. Accounts are created and kept in sync
on every deploy — **no need to wipe anything to add a person.** (Removing a line doesn't delete
that account, to protect any records it owns; wipe the volume if you want a full reset.)

### Setting passwords

Set each person's password as a Railway **Variable** named `PF_PW_` + their username in capitals:

```
PF_PW_SARAH = a-strong-password
PF_PW_MARK  = a-strong-password
PF_PW_TOM   = a-strong-password
```

Add or change one, redeploy, and it takes effect. This keeps passwords out of your code and
GitHub. Any account without its variable falls back to the password in the file (and the startup
log warns you which accounts are still on a default).
