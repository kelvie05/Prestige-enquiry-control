...
Each person signs in with their own account so the audit trail stays accurate. For this
prototype the password is just the username — **change these before any real use.**

| Username | Password | Name        | Role        |
|----------|----------|-------------|-------------|
| sarah    | sarah    | Sarah Chen  | Admin       |
| mark     | mark     | Mark Reilly | Estimator   |
| priya    | priya    | Priya Patel | Estimator   |
| dave     | dave     | Dave Holt   | Sales/Admin |
| janet    | janet    | Janet Cole  | Management  |

You can switch user from the top-right menu to demonstrate shared working and the audit
trail — it performs a real re-login each time.
...
The very first time someone logs in, the sample demo dataset is loaded into the database
so the board isn't empty. After that, everything you do is saved.
...
- **Set the `PF_PW_*` passwords** (step 4) so the well-known demo logins don't work.
...
- The five seeded users are fixed; there's no self-service sign-up or password-reset screen yet.
